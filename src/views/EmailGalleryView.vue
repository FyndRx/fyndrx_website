...
<template>
  <div class="min-h-screen bg-[#F8FAFC] pt-24 pb-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Header -->
      <div class="mb-12">
        <h1 class="text-4xl font-extrabold text-[#0F172A] tracking-tight mb-2">Email Gallery</h1>
        <p class="text-lg text-gray-500">Premium communication templates for the FyndRx ecosystem.</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-3 space-y-4">
          <div 
            ...
            :class="selectedId === template.id 
              ? 'bg-white border-[#246BFD] shadow-lg shadow-blue-500/10' 
              : 'bg-white border-transparent hover:border-gray-200 hover:shadow-md'"
          >
            <div class="flex items-center justify-between mb-1">
              <h3 class="font-bold transition-colors" :class="selectedId === template.id ? 'text-[#246BFD]' : 'text-gray-900'">
                {{ template.name }}
              </h3>
        ...
        <div class="lg:col-span-9">
          <div class="bg-[#0F172A] rounded-[2.5rem] p-4 shadow-2xl overflow-hidden ring-8 ring-gray-100">
            <!-- Simulator Header -->
          ...
          <div class="mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4">
            <div>
              <span class="text-[10px] uppercase tracking-widest font-bold text-[#246BFD] bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Active Template</span>
              <h2 class="text-2xl font-bold text-gray-900 mt-2">{{ selectedTemplate.name }}</h2>
            </div>
            <div class="flex gap-3">
              <button class="px-6 py-3 bg-white border border-gray-200 rounded-xl font-bold text-gray-700 hover:bg-gray-50 transition-colors">
                Copy HTML
              </button>
